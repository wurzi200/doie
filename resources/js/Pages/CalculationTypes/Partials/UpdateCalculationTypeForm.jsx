...
export default function UpdateCalculationType({ calculationType }) {

  const { data, setData, put, errors, processing, recentlySuccessful } = useForm({
    name: calculationType.name,
    minCost: calculationType.minCost,
    maxCost: calculationType.maxCost,
    minSpecial: calculationType.minSpecial,
    maxSpecial: calculationType.maxSpecial,
    minResidual: calculationType.minResidual,
    maxResidual: calculationType.maxResidual,
    minInterest: calculationType.minInterest,
    maxInterest: calculationType.maxInterest,
    minDuration: calculationType.minDuration,
    maxDuration: calculationType.maxDuration,
  ...
  const submit = (e) => {
    ...
    put(route('calculationType.update', calculationType.id));
  };
  ...
  return (
    <form onSubmit={submit} className={`mt-6 space-y-6`}>
      <section className={`p-4 sm:p-8 ${backgroundSecondary} ${border} border shadow sm:rounded-lg`}>
        <header>
          <h2 className={`text-lg font-medium ${textMain}`}>Update Calculation Type</h2>
